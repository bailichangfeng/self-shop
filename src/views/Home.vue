<template>
  <div>
    <header class="home-header wrap" :class="{ active: state.headerScroll }">
      ...
      <router-link v-if="!state.isLogin" class="login" to="./login">登录</router-link>
    ...
    <div class="good">
      ...
      <van-skeleton title :row="3" :loading="state.loading">
        <div class="good-box">
          <div v-for="item in state.newGoodses" :key="item.goodsId" class="good-item" @click="goToDetail(item)">
    ...
    <div class="good">
      ...
      <van-skeleton title :row="3" :loading="state.loading">
        <div class="good-box">
          <div v-for="item in state.hots" :key="item.goodsId" class="good-item" @click="goToDetail(item)">
            <img :src="$filters.prefix(item.goodsCoverImg)" alt="" />
            <div class="good-desc">
              <div class="title">{{ item.goodsName }}</div>
              <div class="price">¥ {{ item.sellingPrice }}</div>
    ...
    <div class="good" :style="{ paddingBottom: '100px' }">
      ...
      <van-skeleton title :row="3" :loading="state.loading">
        <div class="good-box">
          <div v-for="item in state.recommends" :key="item.goodsId" class="good-item" @click="goToDetail(item)">
            <img :src="$filters.prefix(item.goodsCoverImg)" alt="" />
            <div class="good-desc">
              <div class="title">{{ item.goodsName }}</div>
              <div class="price">¥ {{ item.sellingPrice }}</div>
...
interface ShopInfo {
  swiperList: Carousel[]; // 轮播图列表
  isLogin: boolean; // 是否已登录
  headerScroll: boolean; // 滚动透明判断
  hots: HotGoods[]; // 其他属性保留默认类型
  newGoodses: NewGoods[]; // 新品列表
  recommends: RecommendGoods[]; // 推荐商品列表
  categoryList: {
    name: string;
    imgUrl: string;
    categoryId: number;
...
onMounted(async () => {
  ...
  const { data } = await getHome<ResponseData["data"]>();
...
const goToDetail = (item: { goodsId: number }) => {
